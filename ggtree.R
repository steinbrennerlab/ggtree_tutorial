...
dir<-paste("C:/Users/Adam/Dropbox/github/figure_examples/data/ggtree")
tree <- read.tree(paste(dir,"/",opt$entry,sep=""))

dd <- read.table(paste(dir,"/attribute_species.txt",sep=""), header = TRUE, stringsAsFactor=F)

counts_file <- read.table(paste(dir,"/log2foldchanges.txt",sep=""), sep="\t", row.names = 1, header = TRUE, stringsAsFactor=F)
...
p <- ggtree(tree, size=0.1) #size specifies line size
###OPTIONAL: adds hmm coding to ggtree object q
###OPTIONAL: takes hmm_coding and adds to a separate dataframe dd2
dd2 <- read.table(paste(dir,"/attribute_hmm.txt",sep=""), sep="\t", header = TRUE, stringsAsFactor=F)
p <- p %<+% dd2
...
p <- p %<+% dd + geom_tiplab(size=size,offset=0.05,aes(color=species)) + geom_tippoint(aes(size=size2,shape=hmm)) + scale_size_identity() #you need scale_size_identity! https://groups.google.com/forum/#!topic/bioc-ggtree/XHaq9Sk3b00
...
file <- paste(dir,"/output/",opt$output,".pdf",sep="")
...
file_csv <- paste(dir,"/output/",opt$output,".csv",sep="")
